...
public class SwerveMath {
   ...
   */
  public static SimpleMotorFeedforward createDriveFeedforward(Measure<Voltage> optimalVoltage,
      Measure<Velocity<Distance>> maxSpeed,
      double wheelGripCoefficientOfFriction) {
    double kv = optimalVoltage.in(Volts) / maxSpeed.in(MetersPerSecond);
    /// ^ Volt-seconds per meter (max voltage divided by max speed)
    double ka = optimalVoltage.in(Volts)
   ...
   */
  public static Measure<Velocity<Angle>> calculateMaxAngularVelocity(
      Measure<Velocity<Distance>> maxSpeed, double furthestModuleX, double furthestModuleY) {
    return RadiansPerSecond
        .of(maxSpeed.in(MetersPerSecond) / (new Rotation2d(furthestModuleX, furthestModuleY).getRadians()));
  }
   ...
   */
  public static Measure<Velocity<Velocity<Distance>>> calculateMaxAcceleration(
    ...
    return MetersPerSecondPerSecond
        .of((stallTorqueNm * gearRatio * moduleCount) / ((wheelDiameter.in(Meters) / 2) * robotMass.in(Kilograms)));
  }
  ...
  private static double calcMaxAccel(
    ...
    Translation3d robotCG = centerMass.div(robotMass.in(Kilograms));
   ...
   * returning 742 deg.
   * <p>
   * A more formal definition: returns the closest angle {@code n} to
   * {@code scopeReference} such that {@code n} is
   ...
   */
  public static Measure<Angle> placeInAppropriate0To360Scope(Measure<Angle> scopeReference, Measure<Angle> newAngle) {
    // Figure out how many revolutions from the angle to the reference
    double diffRevs = Math.round((scopeReference.in(Degrees) - newAngle.in(Degrees)) / 360) * 360;

    // Add that many revolutions
    return newAngle.plus(Degrees.of(diffRevs));
  }
   ...
   */
  public static void antiJitter(SwerveModuleState moduleState, SwerveModuleState lastModuleState,
      Measure<Velocity<Distance>> maxSpeed) {
    if (Math.abs(moduleState.speedMetersPerSecond) <= (maxSpeed.in(MetersPerSecond) * 0.01)) {
