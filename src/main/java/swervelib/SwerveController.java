...
public class SwerveController {
  ...
  /**
   * {@link SwerveControllerConfiguration} object storing data to generate the
   * {@link PIDController} for controlling the
  ...
  /**
   * Construct the SwerveController object which is used for determining the
   * speeds of the robot based on controller
   * input.
   *
   * @param cfg {@link SwerveControllerConfiguration} containing the PIDF
   *            variables for the heading PIDF.
   */
  ...
  /**
   * Helper function to get the {@link Translation2d} of the chassis speeds given
   * the {@link ChassisSpeeds}.
   *
  ...
  /**
   * Add slew rate limiters to all controls. This prevents the robot from ramping
   * up too much. To disable a
   ...
   * @param angle The {@link SlewRateLimiter} for the angular velocity in
   *              radians/second.
   */
   ...
   *
   * @param x The x value for the joystick in which the deadband should be
   *          applied.
   * @param y The y value for the joystick in which the deadband should be
   *          applied.
  ...
  public ChassisSpeeds getTargetSpeeds(
    ...
    double x = xInput * maxSpeed.in(MetersPerSecond);
    double y = yInput * maxSpeed.in(MetersPerSecond);
   ...
   *                                   robot.
   * @param currentHeadingAngleRadians The current robot heading in radians.
   * @param maxSpeed                   Maximum speed of the drive motors in meters
   *                                   per second, multiplier of the xInput
   *                                   and yInput.
   * @return {@link ChassisSpeeds} which can be sent to the Swerve Drive.
   */
  public ChassisSpeeds getTargetSpeeds(
      ...
      Measure<Velocity<Distance>> maxSpeed) {
    // Converts the horizontal and vertical components to the commanded angle, in
    // radians, unless
    // the joystick is near
    // the center (i. e. has been released), in which case the angle is held at the
    // last valid
    // joystick input (hold
    // position when stick released).
    double angle = withinHypotDeadband(headingX, headingY) ? lastAngleScalar : Math.atan2(headingX, headingY);
  ...
  /**
   * Get the {@link ChassisSpeeds} based of raw speeds desired in meters/second
  ...
  public ChassisSpeeds getRawTargetSpeeds(double xSpeed, double ySpeed, double targetHeadingAngleRadians,
    ...
    return getRawTargetSpeeds(xSpeed, ySpeed,
        thetaController.calculate(currentHeadingAngleRadians, targetHeadingAngleRadians) *
            config.maxAngularVelocity.in(RadiansPerSecond));
  }

  /**
   * Calculate the angular velocity given the current and target heading angle in
   * radians.
   *
   * @param currentHeadingAngle The current heading of the robot in
   *                            radians.
   * @param targetHeadingAngle  The target heading of the robot in radians.
   * @return Angular velocity in radians per second.
   */
  public Measure<Velocity<Angle>> headingCalculate(Measure<Angle> currentHeadingAngle,
      Measure<Angle> targetHeadingAngle) {
    return RadiansPerSecond
        .of(thetaController.calculate(currentHeadingAngle.in(Radians), targetHeadingAngle.in(Radians))
            * config.maxAngularVelocity.in(RadiansPerSecond));
  }

  /**
   * Set a new maximum angular velocity that is different from the auto-generated
   * one. Modified the
   * {@link SwerveControllerConfiguration#maxAngularVelocity} field which is used
   * in the {@link SwerveController} class
   * for {@link ChassisSpeeds} generation.
   *
   * @param angularVelocity Angular velocity in radians per second.
   */
  public void setMaximumAngularVelocity(Measure<Velocity<Angle>> angularVelocity) {
