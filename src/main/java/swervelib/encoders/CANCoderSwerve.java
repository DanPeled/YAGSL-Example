...
public class CANCoderSwerve extends SwerveAbsoluteEncoder {
  ...
  /**
   * An {@link Alert} for if the CANCoder reading is faulty and the reading is
   * ignored.
   */
  ...
  /**
   * Get the absolute position of the encoder. Sets
   * {@link SwerveAbsoluteEncoder#readingError} on erroneous readings.
   *
  ...
  public boolean setAbsoluteEncoderOffset(Measure<Angle> offset) {
    ...
    error = cfg.apply(magCfg.withMagnetOffset(offset.in(Degrees) / 360));
  ...
  public Measure<Velocity<Angle>> getVelocity() {
    return DegreesPerSecond.of(encoder.getVelocity().getValue() * 360);
  }
}
