...
/**
 * {@link swervelib.parser.SwerveModulePhysicalCharacteristics} parsed data.
 * Used to configure the SwerveModule.
 */
public class PhysicalPropertiesJson {

  /**
   * Conversion factor applied to the motor controllers PID loops. Can be
   * calculated with
  ...
  /**
   * The minimum number of seconds to take for the motor to go from 0 to full
   * throttle.
   */
  public MotorConfigDouble rampRate = new MotorConfigDouble(0.25, 0.25);
  /**
   * The grip tape coefficient of friction on carpet. Used to calculate the
   * practical maximum acceleration.
   */
  ...
  public SwerveModulePhysicalCharacteristics createPhysicalProperties() {
    ...
    return new SwerveModulePhysicalCharacteristics(
        ...
        Volts.of(optimalVoltage),
        Amps.of(currentLimit.drive),
        Amps.of(currentLimit.angle),
        Seconds.of(rampRate.drive),
        Seconds.of(rampRate.angle));
  }
}
