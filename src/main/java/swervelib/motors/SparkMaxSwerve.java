...
public class SparkMaxSwerve extends SwerveMotor {
   ...
   */
  @Override
  public void setVoltageCompensation(Measure<Voltage> nominalVoltage) {
    configureSparkMax(() -> motor.enableVoltageCompensation(nominalVoltage.in(Volts)));
  }
  ...
  public void setCurrentLimit(Measure<Current> currentLimit) {
    configureSparkMax(() -> motor.setSmartCurrentLimit((int) currentLimit.in(Amps)));
  }
   ...
   */
  @Override
  public void setLoopRampRate(Measure<Time> rampRate) {
    configureSparkMax(() -> motor.setOpenLoopRampRate(rampRate.in(Seconds)));
    configureSparkMax(() -> motor.setClosedLoopRampRate(rampRate.in(Seconds)));
  }
  ...
  public void configureIntegratedEncoder(double positionConversionFactor) {
    if (absoluteEncoder == null) {
      ...
      // Changes the measurement period and number of samples used to calculate the
      // velocity for the intergrated motor controller
      // Notability this changes the returned velocity and the velocity used for the
      // onboard velocity PID loop (TODO: triple check the PID portion of this
      // statement)
      ...
      // and tested on 9176 for YAGSL, notably 3005 uses 16ms instead of 10 but 10 is
      // more common based on github searches
      ...
      // By default the SparkMax relays the info from the duty cycle encoder to the
      // roborio every 200ms on CAN frame 5
      // This needs to be set to 20ms or under to properly update the swerve module
      ...
      // Some of the frames can probably be adjusted to decrease CAN utilization, with
      // 65535 being the max.
      // From testing, 20ms on frame 5 sometimes returns the same value while
      // constantly powering the azimuth but 8ms may be overkill,
      // with limited testing 19ms did not return the same value while the module was
      // constatntly rotating.
      if (absoluteEncoder.getAbsoluteEncoder() instanceof AbsoluteEncoder) {
        configureCANStatusFrames(100, 20, 20, 200, 20, 8, 50);
      }
      // Need to test on analog encoders but the same concept should apply for them,
      // worst thing that could happen is slightly more can use
